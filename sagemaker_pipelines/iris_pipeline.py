...
def get_sagemaker_pipeline(
    ...
    get_data_step = ProcessingStep(
        ...
        outputs=[ProcessingOutput(source="/opt/ml/processing/output", destination=f"s3://{default_bucket}/iris-data/raw", output_name="output")],
    ...
    preprocess_step = ProcessingStep(
        ...
        inputs=[ProcessingInput(source=get_data_step.properties.ProcessingOutputConfig.Outputs["output"].S3Output.S3Uri, destination="/opt/ml/processing/input")],
        outputs=[
    ...
    split_data_step = ProcessingStep(
        ...
        inputs=[ProcessingInput(source=preprocess_step.properties.ProcessingOutputConfig.Outputs["output"].S3Output.S3Uri, destination="/opt/ml/processing/input")],
        outputs=[ProcessingOutput(source="/opt/ml/processing/output", destination=f"s3://{default_bucket}/iris-data/split", output_name="output")],
    ...
    train_step = TrainingStep(
        ...
        inputs={"training": sagemaker.inputs.TrainingInput(
    ...
    evaluate_step = ProcessingStep(
        ...
        outputs=[ProcessingOutput(source="/opt/ml/processing/output", destination=f"s3://{default_bucket}/iris-artifacts/evaluation_report", output_name="evaluation")],
    ...
    register_model_step = RegisterModel(
        ...
        inference_instances=[parameter_train_job],
        transform_instances=[parameter_train_job],
        ...
        model_metrics={
            "ModelQuality": {
                "Statistics": {
                    ...
                    "S3Uri": evaluate_step.properties.ProcessingOutputConfig.Outputs["evaluation"].S3Output.S3Uri
                }
            }
        }
    )
    ...
    pipeline = Pipeline(
        ...
        parameters=[pipeline_model_accuracy_threshold, pipeline_sample_rate_for_batch_predict],
        steps=[get_data_step, preprocess_step, split_data_step, train_step, evaluate_step, condition_step],
        sagemaker_session=sagemaker_session
    )
    return pipeline
